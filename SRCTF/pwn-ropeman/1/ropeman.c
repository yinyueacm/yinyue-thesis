...
char * decode(char * token){
    char password[16] = "invalid pointer";
    char* result = calloc(sizeof(char), 50);
    int i;
    strncpy(result, token, strlen(token));
    for(i = 0; i<strlen(token); i++){
        result[i] ^= ~password[i % 16];
    }
    return result;
}
...
int check_auth(char *password) {
    int auth_flag = 0;
    //char password_buffer[BSIZE];
    char canary[4] = CVAL;
    char password_buffer[BSIZE];
    //char auth_flag[6] = {0};
    //int auth_flag = 0;
    //printf("%p, %p, %p, %p\n", auth_flag, canary, password_buffer, password);
    strcpy(password_buffer, password);
    char * encode = decode("\xe5\xf4\xea\xfa\xf2\xe1\xfc");
    if(strncmp(canary, CVAL, 4) != 0) {
        puts("BUFFER OVERFLOW DETECTED!");
        exit(1);
    }
    if(strcmp(password_buffer, encode) == 0){
        //memcpy(auth_flag , "1234", 4);
        auth_flag = 1;
    }
    //printf("%s\n", auth_flag);
    return auth_flag;
